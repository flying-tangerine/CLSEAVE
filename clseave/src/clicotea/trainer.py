...
def train_token_alignment(
    ...
    # with a scheduler
    lr_scheduler = set_scheduler(args, optimizer, train_loader, logger) # constant
    ...
    def check_for_nan_inf(engine):
        for name, param in engine.state.model.named_parameters():
            if param.grad is not None:
                if torch.isnan(param.grad).any() or torch.isinf(param.grad).any():
                    engine.state.logger.error(f"NaN or Inf detected in gradients of parameter: {name}")
                    engine.terminate()
                    return
            if torch.isnan(param).any() or torch.isinf(param).any():
                engine.state.logger.error(f"NaN or Inf detected in parameter: {name}")
                engine.terminate()
                return
            
    trainer.add_event_handler(Events.ITERATION_COMPLETED, check_for_nan_inf)
