...
def pad_tensor(original_tensor, target_size):
    padded_tensor = torch.zeros(target_size)
    padded_tensor[:original_tensor.shape[0]] = original_tensor
    return padded_tensor
def pad_tensor_2d(original_tensor, target_shape):
    padded_tensor = torch.zeros(target_shape)  # 创建目标形状的零值张量
    padded_tensor[:, :original_tensor.shape[1]] = original_tensor  # 将原始张量的值复制到前512列
    return padded_tensor
...
class CLIPVEmodel(nn.Module):
    def __init__(self, clicoteackpt, clipve_ckpt):
        ...
        student_state_dict = {
            ...
            if k.startswith("student_model.") #载入没有final_layer_norm和text_projection的老模型
        }
        self.embed.load_state_dict(student_state_dict, strict=False)

        module_dict = {}
        state_dict = torch.load(clipve_ckpt, map_location=torch.device('cpu'))['model_state_dict']
        print(state_dict.get('clip.text_model.final_layer_norm.weight').size())
        print(state_dict.get('clip.text_model.final_layer_norm.bias').size())
        print(state_dict.get('clip.text_projection.weight').size())
        module_dict['final_layer_norm.weight'] = pad_tensor(state_dict.get('clip.text_model.final_layer_norm.weight'), 768)
        module_dict['final_layer_norm.bias'] = pad_tensor(state_dict.get('clip.text_model.final_layer_norm.bias'), 768)
        module_dict['text_projection.weight'] = pad_tensor_2d(state_dict.get('clip.text_projection.weight'), (512, 768))
        self.embed.load_state_dict(module_dict, strict=False)
